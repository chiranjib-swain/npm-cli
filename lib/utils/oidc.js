...
async function oidc ({ packageName, registry, opts, config }) {
  ...
  try {
    ...
    log.verbose('oidc', `[DETECT] CI environment detected: GITHUB_ACTIONS=${ciInfo.GITHUB_ACTIONS}, GITLAB=${ciInfo.GITLAB}, CIRCLE=${ciInfo.CIRCLE}`)
     ...
     */
    // REMOVE this line to simulate the buggy older version:
    // opts[authTokenKey] = response.token   ← delete this line
    config.set(authTokenKey, response.token, 'user')
    log.verbose('oidc', `[DETECT] OIDC token set for ${authTokenKey}`)
