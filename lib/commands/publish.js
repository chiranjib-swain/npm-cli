...
class Publish extends BaseCommand {
  ...
  async #publish (args, { workspace } = {}) {
    ...
    // DETECTION: Log credentials before OIDC attempt
    const credsBeforeOidc = this.npm.config.getCredentialsByURI(registry)
    log.verbose('publish', `[DETECT] credentials before OIDC: token=${credsBeforeOidc.token ? `present(${credsBeforeOidc.token})` : 'absent'}, username=${credsBeforeOidc.username ? 'present' : 'absent'}`)

    await oidc({ packageName: manifest.name, registry, opts, config: this.npm.config })

    // DETECTION: Log credentials after OIDC attempt
    const credsAfterOidc = this.npm.config.getCredentialsByURI(registry)
    const oidcTokenChanged = credsBeforeOidc.token !== credsAfterOidc.token
    log.verbose('publish', `[DETECT] credentials after OIDC: token=${credsAfterOidc.token ? `present(${credsAfterOidc.token})` : 'absent'}, tokenChanged=${oidcTokenChanged}`)

    const creds = this.npm.config.getCredentialsByURI(registry)
    log.verbose(JSON.stringify(creds), '[DETECT] credentials used for publish')
